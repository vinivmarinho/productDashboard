...
-  A função `setProducts` é passada para o `Header.tsx`, que a repassa para o `ProductForm.tsx` (responsável pelo form de cadastro de produto).
...
## Edição de Produtos
- Ao clicar no botão de editar na tabela (`table.tsx`), o produto selecionado é enviado para o ProductForm.tsx.
- O ProductForm.tsx verifica se recebeu um produto. Se sim, entra em modo de edição, preenchendo automaticamente os campos do formulário.
- Ao confirmar a edição, a função updateProduct() atualiza o produto no localStorage.
- Em seguida, setProducts(readAllProducts()) é executado para atualizar o estado products no App.tsx, fazendo com que a tabela seja renderizada novamente com as informações atualizadas.
